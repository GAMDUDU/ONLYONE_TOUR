...
<body>
	    ...
	    <c:set var="alist" value="${aList }" />
          ...
          <div class="row">
			  <div class="alert alert-info" role="alert" style="font-size:20px;">
			  	✈️ 수정하실 항공권 정보를 입력해 주십시오.
			  </div>
	   	   </div>	
		 
       <div class="row">
       <form method="post" class="form-inline" id="air_modify"
	   		   action="<%=request.getContextPath() %>/admin_air_modify_ok.do">
	   
	   	  <input type="hidden" name="air_num" value="${dto.air_num }">
	   	  <input type="hidden" name="page" value="${Page }"> 
	   	  
	      <table class="table table-bordered">
	         <tr>
	            <th class="active" style="vertical-align:middle;">항공이름</th>
	            <td> <input name="air_name" class="form-control"
	            	 style="width:200px;" value="${dto.air_name }" > 
	           	</td>
	         </tr>
					 ...
					 <select class="form-control" style="width:200px;" name="air_airline">
	                  ...
	                  <c:if test="${!empty alist }">
	                     <c:forEach items="${alist }" var="adto">
	                        <c:if test="${dto.air_airline == adto.air_airline }">
	                           <option value="${adto.air_airline }" selected>${adto.air_airline }</option>
	                        </c:if>
	                        
	                        <c:if test="${dto.air_airline != adto.air_airline }">
	                           <option value="${adto.air_airline }">${adto.air_airline }</option>
	                        </c:if>
	                     </c:forEach>
	                  </c:if>
	               </select>
					...
					<select class="form-control" style="width:200px;" name="air_departure">
	                  ...
	                  <c:if test="${!empty nlist }">
	                     <c:forEach items="${nlist }" var="ndto">
	                        <c:if test="${dto.air_departure == ndto.air_nation_val }">
	                           <option value="${ndto.air_nation_val }" selected>
	                           				${ndto.air_nation}</option>
	                        </c:if>
	                        
	                        <c:if test="${dto.air_departure != ndto.air_nation_val }">
	                           <option value="${ndto.air_nation_val }">
	                           		${ndto.air_nation}</option>
	                        </c:if>
	                     </c:forEach>
	                  </c:if>
	         ...
	         <tr>
	         	...
	         	<td>
					<select class="form-control" style="width:200px;" name="air_arrival">
	                  ...
	                  <c:if test="${!empty nlist }">
	                     <c:forEach items="${nlist }" var="ndto">
	                        <c:if test="${dto.air_arrival == ndto.air_nation_val }">
	                           <option value="${ndto.air_nation_val }" selected>
	                           				${ndto.air_nation}</option>
	                        </c:if>
	         ...
	         <tr>
	            ...
	            <td> 
	            	<input name="air_date" type="date" placeholder="출발날짜" 
	            	class="form-control" style="width:200px;" value="${dto.air_date }">
	            </td>
	         </tr>
	         	         
	         <tr>
	            <th class="active" style="vertical-align:middle;">출발시간</th>
	            <td> <input type="time" name="air_departure_date" class="form-control"
	            	 style="width:200px;" value="${dto.air_departure_date }"> </td>
	         </tr>
	         	         
	         <tr>
	            <th class="active" style="vertical-align:middle;">도착시간</th>
	            <td> <input type="time" name="air_arrival_date" class="form-control" 
	            	style="width:200px;" value="${dto.air_arrival_date }"> </td>
	         </tr>
	         
	         <tr>
	            <th class="active" style="vertical-align:middle;">성인요금</th>
	            <td><label style="font-size:18px;">
	            	<input name="air_adult_fare" class="form-control"
	            	 style="width:200px;" value="${dto.air_adult_fare }">원</label></td>
	         </tr>
	         	         
	         <tr>
	            <th class="active" style="vertical-align:middle;">소아요금</th>
	            <td><label style="font-size:18px;">
	            	<input name="air_child_fare" class="form-control"
	            	style="width:200px;" value="${dto.air_child_fare }">원</label></td>
	         </tr>
	         
	         <tr>
	            <td colspan="2" align="center">
	               <input type="button" value="항공권 수정" class="btn btn-primary" id="modifyAir">
	               &nbsp;&nbsp;&nbsp;
   ...
   <script type="text/javascript">
     ...
     $("#modifyAir").click(function() {
			...
			var result = confirm('항공권을 수정하시겠습니까?');
